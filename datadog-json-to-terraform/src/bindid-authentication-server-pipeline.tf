resource "datadog_logs_custom_pipeline" "resource_name" {
...
EOF
  ...
  processor {
    grok_parser {
      ...
      is_enabled = true
      source     = "message"
      samples    = ["2021-05-27 14:49:30.341 INFO  c.t.c.a.a.t.WaitForTicketActionAssertion:29 - 3456111712272389920 142118441788883014 - [default] [64503d_405049] [BID_386d-4213-4d16-b6fe-60ca90b8cc35] [transmit-akka.actor.default-dispatcher-5] Ticket was consumed, collecting reply; tId [d603daf3-8e08-4a3d-960e-5cbd26e767a8], uid [58d21530-71be-4b09-b13d-df5f3fdfd30c, params [{\"system_params\":{\"uid\":\"58d21530-71be-4b09-b13d-df5f3fdfd30c\"},\"user_params\":{\"bind_id\":{\"ama\":{\"response\":{\"result\":\"ko\",\"is_foreign_error\":true,\"error_id\":\"ama_user_cancelled_authentication\",\"description\":\"Authentication cancelled by the user\",\"code\":\"access_denied\"}}}}}]", "2021-05-25 05:47:33.982 ERROR c.t.s.r.Routes:273 - 3415527942269905757 3668766704050484136 - [default] [04eeb4_835422] [BID_2cb5-a7d1-4424-9de2-08d451f31cd6] [transmit-akka.actor.default-dispatcher-6] Request rejected; method: [POST], uri: [/api/v2/auth/assert?aid=bindid-oidc&did=19f1b690-c486-47ce-8f2e-9a9ec72e455b&sid=4f2ba426-a326-47c3-a26d-0709efbaa8a4&locale=en-US], statusCode: [401], errorCode: [4001], errorMessage: [Session rejected 'd7ed6c61-b523-4df1-b8a4-5953661cde12'], data: [Some(RejectedAuthenticationResponseData(None,Some({\"source\":{\"action_type\":\"reject\",\"type\":\"action\"},\"reason\":{\"type\":\"assertion_rejected\",\"data\":{\"result\":\"ko\",\"code\":\"access_denied\",\"description\":\"Authentication cancelled by the user\",\"errorId\":\"desktop_consent_rejected\",\"is_foreign_error\":false}}}),Some({\"consent\":\"reject\"})))], sdk headers: [List(SessionStateSdkHeader(Closed))], millis: [39]", ]
      grok {
        support_rules = ""
        match_rules   = "ruleParseErrorId1 %%{data:}(\"error_id\"|\"errorId\")\\:\"%%{data:error_id}\"%%{data:}"
      }
    }
  }
  processor {
  ...
  processor {
    category_processor {
      ...
      category {
        filter {
          query = "@message:*\\\"is_foreign_error\\\"\\:true*"
        }
        name = "Foreign errors"
      }
      category {
      ...
      category {
        filter {
          query = "@uri:\\/api\\/v2\\/oidc\\/bindid-oidc\\/authorize\\?* @message:*invalid_request* @message:*Missing?* @message:*client_id*"
        }
      ...
      category {
        filter {
          query = "@message:*\\\"assertion_error_code\\\"\\:9* OR @message:*FieldMissingFromRequest\\(attestation_credential\\)*"
        }
      ...
      category {
        filter {
          query = "@statusCode:500 @uri:(\\/api\\/v2\\/auth\\/* OR \\/api\\/v2\\/oidc\\/bindid-oidc\\/complete) -@message:*rejection\\:?MethodRejection* -@message:*client?returned?with?error* -@message:*cannot_consume_ticket*"
        }
        name = "500: End-Users"
      }
      category {
      ...
      category {
        filter {
          query = "(@statusCode:500 OR @status:500) -@uri:\\/api\\/v2\\/auth\\/* -@uri:\\/api\\/v2\\/oidc\\/bindid-oidc\\/complete -@message:*rejection\\:?MethodRejection* -@message:*client?returned?with?error* -@message:*cannot_consume_ticket*"
        }
        name = "500: Backend & Management"
      }
      category {
        filter {
          query = "@api_audience:enduser (@statusCode:400 OR @statusCode:401 OR @statusCode:404 OR @status:(400 OR 401)) @message:*User?not?found*"
        }
      ...
      category {
        filter {
          query = "@message:*invalid_request*"
        }
        name = "Invalid Request"
      }
      category {
        filter {
          query = "@message:*\\\"failureCommand\\\"\\:\\\"restart_journey\\\"*"
        }
        name = "Errors Ignored"
      }
